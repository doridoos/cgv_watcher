...
CGV는 비정상 접속을 감지해 **IP 단위로 차단**합니다. 차단되면 브라우저로 접속해도
"비정상적으로 CGV에 접속한 것이 확인되어 이용이 제한되었어요" 화면(RAY_ID,
CLIENT_IP 표시)이 뜨고, 그 IP에서는 예매 자체가 막힙니다.

그래서 이렇게 쓰세요:

- **회사·공용 네트워크에서 돌리지 마세요.** 차단은 IP 단위라 같은 네트워크를 쓰는
  동료 전체가 CGV를 못 쓰게 됩니다. 개인 네트워크에서만 실행하세요.
- **폴링 간격을 여유롭게.** 기본값은 10분(`interval_sec: 600`)이고 버스트도 3분이
  하한입니다. 더 짧게 줄이지 마세요 — 취소표는 몇 분 단위로 왔다 갔다 하므로
  10분 간격으로도 충분히 잡힙니다.
- **감시 대상은 좁게.** 날짜 범위와 시간창을 좁히면 요청 수가 줄어듭니다
  (하루 = 요청 1회).
- **차단됐다면** 감시를 멈추고 몇 시간~하루 기다리세요. 차단이 풀리지 않으면
  고객센터(1544-1122) 문의가 유일한 방법입니다.
